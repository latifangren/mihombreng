...
type ParseResponse struct {
	...
	Proxies []*proxylib.Proxy `json:"proxies,omitempty"`
...
func (h *ConverterHandler) ParseProxies(c *gin.Context) {
	...
	var proxies []*proxylib.Proxy
	var err error

	if req.Content != "" && req.Content != "string" {
		proxies, err = proxylib.ParseSubscription(req.Content)
	} else if req.URL != "" {
		if strings.HasPrefix(req.URL, "http://") || strings.HasPrefix(req.URL, "https://") {
			proxies, err = proxylib.FetchSubscription(req.URL)
			...
			var proxy *proxylib.Proxy
			proxy, err = proxylib.ParseLink(req.URL)
			if err == nil {
				proxies = []*proxylib.Proxy{proxy}
			}
		} else {
			c.JSON(http.StatusBadRequest, ParseResponse{
				Success: false,
