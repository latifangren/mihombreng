...
- [ ] Routing mode UX and orchestration are incomplete
- [ ] Provider management has a baseline operational workspace, but lacks rich sync/health/validation semantics
- [ ] Subscription workspace baseline is available, but lifecycle automation, scheduler, and activation workflows are not yet core features
- [ ] Traffic / connections observability has traffic metrics v2 + connections workspace, but lacks real-time updates and chart visualizations
...
- [ ] Diagnostics page lacks an advanced health model / recovery actions
- [ ] Health model is basic, lacks recovery metadata
- [x] OpenWrt packaging and CI/CD baseline shipped; pending further distribution polish and routing orchestration
...
- [ ] TUN / Redirect / TProxy / Mixed mode orchestration end-to-end
- [ ] Firewall policy management, cleanup, rollback, and verification flows
- [ ] UI mode switcher, prerequisite checks, and recovery hints

### C. Profiles, Providers, and Config Operations

- [x] Config editor baseline
- [x] Proxy / rule provider CRUD baseline
- [ ] Config validation, diff/revision safety, and activation flow maturity
...
- [ ] Full routing mode orchestration
- [ ] Firewall policy recovery / rollback
- [ ] Connectivity verification flows
...
- [~] M8.1 — Routing Mode Orchestration (deferred)
